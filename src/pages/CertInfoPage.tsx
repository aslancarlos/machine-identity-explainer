...
// ── Real certificate data from machine.minha.cloud ───────────────────────────
...
const AUDIT: AuditItem[] = [
  ...
  },
  {
    ...
    detail: 'Certificate is logged in public CT logs. Allows detection of misissued certificates.',
  ...
  },
  {
    ...
    nist:   'NIST SP 800-57: short-lived certificates reduce the exposure window on compromise.',
  },
  {
    label:  'OCSP Stapling',
    status: 'warn',
  ...
  },
  {
    ...
    detail: 'RSA 4096 is secure but slower than ECDSA P-256, which provides equivalent security (~128-bit) with a 4x smaller key and faster handshakes.',
...
const NIST_ITEMS: NistItem[] = [
  ...
  { ref: 'SP 800-57 Pt 1',  title: 'RSA key 2048 bits or more',          status: 'pass', desc: 'RSA 4096-bit key in use, twice the minimum. Provides 140-bit security strength.' },
...
const BEST_PRACTICES: BestPractice[] = [
  ...
  },
  {
    ...
    detail:   'RSA 4096 is strong but the private-key operations are approximately 40x slower than ECDSA P-256 for equivalent security. P-256 reduces TLS handshake CPU and improves latency, especially on mobile.',
  ...
  },
  {
    ...
    detail:   "Let's Encrypt certificates expire every 90 days. Configure a cron or systemd timer to run certbot renew at least twice a week.",
      ...
      <div className="max-w-5xl mx-auto space-y-16">
        ...
        <motion.div {...fade(0)} className="text-center space-y-4">
          <span className="badge bg-mi-cyan/10 text-mi-cyan border border-mi-cyan/20">
            <Lock size={11} className="mr-1.5" /> {t('certinfo_page.badge')}
          </span>
          <h1 className="text-4xl sm:text-5xl font-bold">machine.minha.cloud</h1>
          <p className="text-slate-400 max-w-xl mx-auto">
            {t('certinfo_page.subtitle')}
          </p>
        </motion.div>
        ...
        <motion.div {...fade(0.05)} className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: t('certinfo_page.score_overall'), value: `${overallPct}%`,                               color: overallPct >= 80 ? 'text-spiffe' : overallPct >= 60 ? 'text-mi-gold' : 'text-mi-red' },
            { label: t('certinfo_page.score_tls'),     value: `${tlsScore.pass}/${tlsScore.total}`,           color: 'text-mi-cyan'  },
            { label: t('certinfo_page.score_headers'), value: `${headerScore.pass}/${headerScore.total}`,     color: 'text-mi-gold'  },
            { label: t('certinfo_page.score_nist'),    value: `${nistScore.pass}/${nistScore.total}`,         color: 'text-pki'      },
          ].map((c, i) => (
        ...
        <motion.div {...fade(0.1)} className="space-y-4">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <FileText size={20} className="text-mi-cyan" /> {t('certinfo_page.section_cert')}
          </h2>
          <div className="rounded-xl border border-border overflow-hidden">
            {[
              { icon: Globe,    label: t('certinfo_page.field_subject'),       value: CERT.subject },
              { icon: Globe,    label: t('certinfo_page.field_sans'),          value: CERT.sans.join(', ') },
              { icon: Shield,   label: t('certinfo_page.field_issuer'),        value: CERT.issuer },
              { icon: Calendar, label: t('certinfo_page.field_valid_from'),    value: CERT.notBefore },
              { icon: Calendar, label: t('certinfo_page.field_valid_until'),   value: `${CERT.notAfter} (${CERT.daysRemaining} ${t('certinfo_page.days_remaining')})`, warn: CERT.daysRemaining < 30 },
              { icon: Key,      label: t('certinfo_page.field_pubkey'),        value: CERT.keyAlgo },
              { icon: Lock,     label: t('certinfo_page.field_sig_algo'),      value: CERT.sigAlgo },
              { icon: Hash,     label: t('certinfo_page.field_serial'),        value: CERT.serial },
              { icon: Hash,     label: t('certinfo_page.field_sha1'),          value: CERT.sha1 },
              { icon: Server,   label: t('certinfo_page.field_tls_version'),   value: CERT.tlsVersion },
              { icon: Lock,     label: t('certinfo_page.field_cipher'),        value: CERT.cipher },
              { icon: Globe,    label: t('certinfo_page.field_key_usage'),     value: CERT.keyUsage.join(', ') },
              { icon: Globe,    label: t('certinfo_page.field_ext_key_usage'), value: CERT.extKeyUsage.join(', ') },
              { icon: Info,     label: t('certinfo_page.field_ct_logs'),       value: t('certinfo_page.ct_value') },
              { icon: Info,     label: t('certinfo_page.field_crl'),           value: CERT.crl },
            ].map((row, i) => {
              const Icon = row.icon
              return (
        ...
        <motion.div {...fade(0.15)} className="space-y-4">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Shield size={20} className="text-mi-gold" /> {t('certinfo_page.section_tls')}
          </h2>
        ...
        <motion.div {...fade(0.2)} className="space-y-4">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Server size={20} className="text-pki" /> {t('certinfo_page.section_headers')}
          </h2>
          <div className="rounded-xl border border-border overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-bg-muted/50">
                  ...
                  <th className="text-left px-5 py-3 text-slate-400 font-semibold">{t('certinfo_page.col_header')}</th>
                  <th className="text-left px-5 py-3 text-slate-400 font-semibold hidden sm:table-cell">{t('certinfo_page.col_value')}</th>
                  <th className="text-left px-5 py-3 text-slate-400 font-semibold w-20">{t('certinfo_page.col_status')}</th>
                </tr>
              </thead>
              <tbody>
                {HEADERS.map((h, i) => (
        ...
        <motion.div {...fade(0.25)} className="space-y-4">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <FileText size={20} className="text-spiffe" /> {t('certinfo_page.section_nist')}
          </h2>
          <p className="text-sm text-slate-500">{t('certinfo_page.section_nist_desc')}</p>
          <div className="space-y-2">
        ...
        <motion.div {...fade(0.3)} className="space-y-4">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <CheckCircle size={20} className="text-mi-cyan" /> {t('certinfo_page.section_recs')}
          </h2>
        ...
        <motion.div {...fade(0.35)} className="section-card space-y-4 bg-gradient-to-br from-mi-cyan/5 to-bg-card border-mi-cyan/20">
          <h2 className="text-lg font-bold text-white">{t('certinfo_page.section_refs')}</h2>
